...
<template>
  <div :class="['rounded-box border border-base-300 bg-base-100', 'transition-all', { 'shadow': open }]">
    <div class="flex items-center px-4 py-2 gap-4" tabindex="0">
      ...
      <button class="btn btn-ghost btn-sm" :disabled="isOpening" @click.stop="() => emit('delete-folder', path)">
        <Trash2 class="w-5 h-5" />
      </button>

      <button class="btn btn-ghost btn-sm p-0 min-w-0" @click="toggle" :aria-expanded="open"
