...
const buttonDisabled = computed(() => [AppState.Verifying, AppState.Downloading].includes(appState.value));
...
watch(appState, (newState) => {
  showSpinner.value = [AppState.Verifying, AppState.CreatingTransaction].includes(newState);
});

watch(downloadProgressData, (progress) => {
  if (!progress) return;
  const fileProgress = (progress.current / progress.file_size) * 100;
  const overallProgress = ((progress.file_index - 1) * 100 + fileProgress) / progress.total_files;
  ...
  <dialog ref="modalRef" id="transaction_modal" class="modal">
    <div class="modal-box">
      ...
      <div v-else>
        ...
        <ul>
          <li v-for="file in transactionReport.outdated_files" :key="file.path">
            {{ file.path }}
            <span v-if="file.current_size" class="opacity-50">
              (Current Size: {{ formatBytes(file.current_size) }}, New Size: {{ formatBytes(file.new_size) }})
            </span>
          </li>
        </ul>
        ...
        <span v-if="transactionReport.disk_space_change > 0">
          After this operation, {{ formatBytes(transactionReport.disk_space_change) }} of additional disk space will be
          used.
        </span>
        <span v-else>
          After this operation, {{ formatBytes(transactionReport.disk_space_change) }} of additional disk space will be
      ...
      <div class="mt-2 flex flex-row">
        <button class="btn btn-primary flex-grow items-center justify-center text-center text-neutral-content"
          @click="handleButtonClick"> {{ buttonLabel }}
        </button>
        <button v-if="transactionReport" class="ml-2 btn btn-md btn-secondary"
          @click="() => modalRef?.close()">Deny</button>
      </div>
    </div>
  </dialog>
  ...
  <div class="p-4 bg-base-200/75">
    <div class="flex items-end">
      <div class="flex flex-col flex-grow">
        <p class="text-lg">
          {{ primaryMessage }} <span v-if="showSpinner" class="loading loading-spinner text-primary"></span>
        </p>
